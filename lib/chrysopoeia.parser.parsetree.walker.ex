defmodule Chrysopoeia.Parser.ParseTree.Walker do
  ...
  @doc ~S""" 
    matches a text node. 
  """
  def _walk(text , _fns, acc, path) when is_binary(text) do
  ...
  def _walk(t = { _, _, [c]}, fns, acc, path) when is_binary(c) do
  ...
  """
  def _walk(t = {e, _a, c}, fns, acc, path) when is_list(c) do
    ...
    unless e == :delete do
      ...
      {children, acc} = Enum.map_reduce(c, [], fn
        (child, r_acc) -> 
          ...
          {t, a} = _walk(child, fns, r_acc, path ++ [{e, a}])
          {t, r_acc ++ a} 
      end)
  ...
  """
  def _walk(t = {_, _, []}, fns, acc, path) do
